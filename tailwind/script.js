...
// Hàm phụ trợ: gỡ 1 nhóm class, thêm 1 nhóm class khác — dùng cho toggle Tailwind
...
async function taiGioHangTuAPI() {
  try {
    ...
    // đồng bộ lại nút "Thêm vào giỏ" trên từng card theo đúng dữ liệu vừa tải
    allCards.forEach(function (card) {
...
// ----- Vẽ lại giao diện giỏ hàng dựa theo dữ liệu hiện có trong biến gioHang -----
...
// Gắn sự kiện cho nút "Thêm vào giỏ" trên từng card
allCards.forEach(function (card) {
  const addBtn = card.querySelector('.btn-add-cart');
  const moduleId = card.dataset.moduleId;
  const gia = Number(card.dataset.price);
